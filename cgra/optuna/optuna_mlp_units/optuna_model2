...
def objective(trial):
    ...
    sys_bits_b0 = SYS_BITS(x=8, k=4, b=16)
    sys_bits_b1 = SYS_BITS(x=4, k=8, b=16)
    ...
    filters_b0 = trial.suggest_categorical("filters_b0", [32, 64, 128, 512])
    ...
    user_model =UserModel(
        ...
        kernel_sizes={"b0": kernel_size_b0},
        filters={"b0": filters_b0},
...
class UserModel(XModel):
    def __init__(self, sys_bits_b0, x_int_bits, sys_bits_b1, kernel_sizes, filters, *args, **kwargs):
...
study = optuna.create_study(study_name="model2_kf_b0(hopefully_final)", direction="minimize", storage=storage)  # Minimizing loss
...
for layer in ["b0"]:
    ...
    fig.write_html(f"model2_kf__optuna_contour_{layer}.html")

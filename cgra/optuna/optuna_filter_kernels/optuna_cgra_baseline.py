...
def objective(trial):
    ...
    sys_bits_b0 = SYS_BITS(x=12, k=8, b=16)
    sys_bits_b1 = SYS_BITS(x=2, k=16, b=16)
    sys_bits_b2 = SYS_BITS(x=16, k=8, b=16)
    sys_bits_b3 = SYS_BITS(x=12, k=16, b=16)
    sys_bits_b4 = SYS_BITS(x=4, k=2, b=16)
    sys_bits_b5 = SYS_BITS(x=16, k=16, b=16)
...
class UserModel(XModel):
    def __init__(self, sys_bits_b0, x_int_bits, sys_bits_b1, sys_bits_b2, 
                 sys_bits_b3, sys_bits_b4, sys_bits_b5, kernel_sizes, filters, *args, **kwargs):
...
study = optuna.create_study(study_name="baseline_kf_b012(hopefully_final)", direction="minimize", storage=storage)  # Minimizing loss
...
for layer in ["b0", "b1", "b2"]:
    ...
    fig.write_html(f"baseline_kf__optuna_contour_{layer}.html")

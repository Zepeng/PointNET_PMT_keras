    ...
    def __init__(self, sys_bits, x_int_bits, *args, **kwargs):
        ...
        self.b0 = XBundle( 
            core=XConvBN(
                ...
                filters=128,
                kernel_size=3,
                act=XActivation(sys_bits=SYS_BITS(x=16, k=16, b=16), o_int_bits=0, type='relu', slope=0)
                ),
            pool=XPool(
                type='avg',
                pool_size=(2126,1),
                strides=(2126,1),
                padding='same',
                act=XActivation(sys_bits=SYS_BITS(x=16, k=16, b=16), o_int_bits=0, type=None),),
            flatten=True
            # core=XDense(
            #    k_int_bits=0,
            #    b_int_bits=0,
...
if training:
    ...
    val_save_data = {
        ...
        'std_x_diff': np.std(x_diff.numpy()),
        'std_y_diff': np.std(y_diff.numpy()),
        'std_z_diff': np.std(z_diff.numpy()),
        'std_energy_diff': np.std(energy_diff.numpy())
    ...
    Avg. abs. diff. in x={abs_x_diff:.2f}, y={abs_y_diff:.2f}, z={abs_z_diff:.2f}, energy={abs_energy_diff:.2f}\n\
        std_x_diff={np.std(x_diff.numpy()):.2f}, y_diff={np.std(y_diff.numpy()):.2f},, z_diff={np.std(z_diff.numpy()):.2f}, std_energy_diff={ np.std(energy_diff.numpy()):.2f}"
        , fontsize=20)
    # else:
    ...
    plt.savefig(f'./model2_cgra_pointnet_hist_with_fit.png')
    plt.close()
    ###########################################
